...
using namespace messaging_proto;
...
static void dump_im_msg(const struct mosquitto_message *msg) {
  ...
  std::cerr << msg->topic << ": " << InstantMessageType_Name(lmt_type) << " id " << id << " len " << msg->payloadlen << '\n';
	switch(lmt_type) {
		case messaging_proto::IM_ADD_CONTACT:
      {
        ImAddContact req;
          ...
          std::cerr << "-> " << req.Utf8DebugString() << '\n';
			...
			break;
		case messaging_proto::IM_SHARE_SOC_ID:
      {
        ImShareSocId req;
				if (!req.ParseFromArray(p + 10, msg->payloadlen - 10)) {
          std::cerr << "msg " << type << " decode error!";
          ...
          std::cerr << "-> " << req.ShortDebugString() << '\n';
			...
			break;
		case messaging_proto::IM_FEEDBACK_MSG:
      {
        ImFeedbackMsg req;
				if (!req.ParseFromArray(p + 10, msg->payloadlen - 10)) {
          std::cerr << "msg " << type << " decode error!";
...
static void dump_mqtt(const struct mosquitto_message *msg) {
  ...
  std::cerr << "recv: " << msg->topic << ": " << LiveMessageType_Name(lmt_type) << " id " << id << " len " << msg->payloadlen << '\n';
	switch(lmt_type) {
		case messaging_proto::LMT_SYSTEM_NOTIFICATION:
      std::cerr << " id " << id << "\n";
			break;
		case messaging_proto::LMT_SERVICE_RESPONSE:
      {
        ServiceResponse req;
				if (!req.ParseFromArray(p + 10, msg->payloadlen - 10)) {
          std::cerr << "msg " << type << " decode error!";
          ...
          std::cerr << "-> " << req.Utf8DebugString() << '\n';
			...
			break;
		case messaging_proto::LMT_NEARBY_USER_UPDATE:
      {
        NearbyUserUpdate req;
          ...
          std::cerr << "-> " << req.ShortDebugString() << '\n';
			...
			break;
		case messaging_proto::LMT_GAME_CONTACT_INFO:
      {
        GameContactInfo req;
				if (!req.ParseFromArray(p + 10, msg->payloadlen - 10)) {
          std::cerr << "msg " << type << " decode error!";
          ...
          std::cerr << "-> " << req.Utf8DebugString() << '\n';
			...
			break;
		case messaging_proto::LMT_GAME_LIVE_STATE:
			{
				messaging_proto::GameLiveStateInfo req;
				if (!req.ParseFromArray(p + 10, msg->payloadlen - 10)) {
          std::cerr << "msg " << type << " decode error!";
				} else {
          std::cerr << "-> " << req.Utf8DebugString() << '\n';
			...
			break;
		case messaging_proto::LMT_LIVE_CONNECT_NOTIFY:
			{
				messaging_proto::LiveConnectNotify req;
				if (!req.ParseFromArray(p + 10, msg->payloadlen - 10)) {
          std::cerr << "msg " << type << " decode error!";
				} else {
          std::cerr << req.Utf8DebugString() << '\n';
			...
			break;
		case messaging_proto::LMT_SERVER_STATUS:
			{
				messaging_proto::ServerStatus req;
				if (!req.ParseFromArray(p + 10, msg->payloadlen - 10)) {
          std::cerr << "msg " << type << " decode error!";
				} else {
          std::cerr << req.Utf8DebugString() << '\n';
...
void send_live_msg_game_state(struct mosquitto *mosq, const char *topic) {
  ...
  for (int i = 0; i < 4; ++i, ++id) {
    messaging_proto::GameLiveStateInfo msg;
    msg.set_state(messaging_proto::game_state_idle);
    ...
    send_mqtt_msg(mosq, topic, messaging_proto::LMT_GAME_LIVE_STATE, id, msg);
  }
}
...
void send_game_contact_msg(struct mosquitto *mosq, const char *topic) {
  ...
  for (int i = 0; i < 1; ++i, ++id) {
    messaging_proto::GameContactInfo msg;
    auto user = msg.mutable_from();
    msg.set_type(messaging_proto::GameContactInfo::interested);
    ...
    *msg.mutable_gps() = msg.gps();
    send_mqtt_msg(mosq, topic, messaging_proto::LMT_GAME_CONTACT_INFO, id, msg);

    /*
    sleep(3);
    msg.set_type(messaging_proto::GameContactInfo::invite);
    user->set_uid(2017004);
...
void send_msg(struct mosquitto *mosq, const char *topic) {
  ...
  for (int i = 0; i < 1; ++i, ++id) {
    ...
    //messaging_proto::ServerStatus msg;
    //msg.set_cmd(1);
    ...
    send_mqtt_msg(mosq, topic, messaging_proto::LMT_SEARCH_NEARBY_USERS, id, msg);
  }
}
