...
static std::thread *g_kafka_thread;
...
#define PGQCONNSTR "postgresql://dba:jO.sAt1G0T@localhost/codein"
...
static void on_client_online(long uid, const char *token, const char *clientid, int sd) {
  ...
  msg.set_type(kafka_proto::MqttStatus::online);
  msg.set_uid(uid);
  msg.set_token(token);
...
int mosquitto_auth_connect_check(void *user_data, const char *username,
      const char *password, const char *clientid, int sd) {
